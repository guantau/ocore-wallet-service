...
describe('Email notifications', function() {
  ...
  describe('1-1 wallet', function() {
          ...
          async.eachSeries(w.copayers, function(copayer, next) {
            ...
            emailService.start({
              ...
              emailOpts: {
                ...
                publicTxUrlTemplate: {
                  obyte: {
                    livenet: 'https://explorer.obyte.org/#{{&txid}}',
                    testnet: 'https://testnet.explorer.obyte.org/#{{&txid}}'
                  }
                }
              },
            }, function(err) {
    ...
    it('should handle small incomming payments', function(done) {
      server.createAddress({}, function(err, address) {
        ...
        server._notify('NewIncomingTx', {
          ...
          setTimeout(function() {
            ...
            var one = emails[0];
            one.from.should.equal('ows@dummy.net');
            one.subject.should.contain('New payment received');
            one.text.should.contain('221,340');
      ...
      helpers.stubUtxos(server, wallet, [1, 1], function() {
        ...
        async.waterfall([
          ...
          function(t, next) {
            ...
            async.eachSeries(_.range(2), function(i, next) {
              ...
              helpers.getAuthServer(copayer.id44, function(server) {
                var signatures = helpers.clientSign(txp, copayer.xPrivKey_44H_0H_0H, server.walletId);
                server.signTx({
                  txProposalId: txp.id,
          ...
          setTimeout(function() {
            ...
            one.text.should.contain('80,000,000');
            should.exist(one.html);
            one.html.should.contain('https://explorer.obyte.org/#' + txp.txid);
            server.storage.fetchUnsentEmails(function(err, unsent) {
              should.not.exist(err);
              unsent.should.be.empty;
              emailService._readTemplateFile = _readTemplateFile_old;
      ...
      server.createAddress({}, function(err, address) {
        ...
        server._notify('NewIncomingTx', {
          ...
          setTimeout(function() {
            ...
            one.from.should.equal('ows@dummy.net');
            one.subject.should.contain('New payment received');
            one.text.should.contain('13,000');
            server.storage.fetchUnsentEmails(function(err, unsent) {
              should.not.exist(err);
              unsent.should.be.empty;
              done();
      ...
      server.createAddress({}, function(err, address) {
        ...
        server._notify('NewIncomingTx', {
          ...
          setTimeout(function() {
            ...
            one.from.should.equal('ows@dummy.net');
            one.subject.should.contain('New payment received');
            one.text.should.contain('12,300,000');
            server.storage.fetchUnsentEmails(function(err, unsent) {
              should.not.exist(err);
              unsent.should.be.empty;
              done();
        ...
        server.txConfirmationSubscribe({
          ...
          server._notify('TxConfirmation', {
            ...
            setTimeout(function() {
              ...
              email.from.should.equal('ows@dummy.net');
        ...
        server.createAddress({}, function(err, address) {
          ...
          server._notify('NewIncomingTx', {
            ...
            setTimeout(function() {
              ...
              spanish.from.should.equal('ows@dummy.net');
              spanish.subject.should.contain('Nuevo pago recibido');
              spanish.text.should.contain('12,300,000');
              var english = _.find(emails, {
                to: 'copayer2@domain.com'
              });
              english.from.should.equal('ows@dummy.net');
              english.subject.should.contain('New payment received');
              english.text.should.contain('12,300,000');
              done();
            }, 200);
          });
        });
        ...
        server._notify('NewIncomingTx', {
          ...
          setTimeout(function() {
            ...
            server.storage.fetchUnsentEmails(function(err, unsent) {
              ...
              unsent[0].from.should.equal('ows@dummy.net');
              unsent[1].from.should.equal('ows@dummy.net');
              unsent[2].from.should.equal('ows@dummy.net');
              done();
            });
          }, 1000);
        });
