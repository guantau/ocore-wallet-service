...
WalletService.prototype.joinWallet = function (opts, cb) {
  ...
  if (!Utils.checkRequired(opts, ['deviceId', 'walletId', 'name', 'xPubKey', 'requestPubKey', 'copayerSignature'], cb)) return;
...
WalletService.prototype.getBalance = function (opts, cb) {
  ...
  if (opts.addresses) {
    blockchainExplorer.getBalance(opts.addresses, opts, function (err, balance) {
      if (err) return cb(err);
    ...
    self.getWallet({}, function (err, wallet) {
      ...
      checkTxpAlreadyExists(opts.txProposalId, function (err, tx) {
        ...
        async.series([
          ...
          function (next) {
            if (opts.app == 'payment' && opts.params.send_all) return next();
 ...
 * @param {Number} opts.limit [Optional] - (defaults to all)
...
WalletService.prototype.getTxs = function (opts, cb) {
  ...
  if (!_.isUndefined(opts.status) && !Utils.checkValueInCollection(opts.status, ['temporary', 'accepted', 'broadcasted', 'rejected'])) {
 ...
 *
 * @param {Object} opts
 * @param {String} opts.asset [Required] - Asset. 'all' for all assets, null and 'base' and 'bytes' for bytes. (defaults to 'all').
 * @param {Number} opts.skip [Optional] - (defaults to 0)
 * @param {Number} opts.limit [Optional] - (defaults to 50)
 * @param {Number} opts.includeExtendedInfo [Optional] - Include all inputs/outputs for every tx. (defaults to fasle).
...
WalletService.prototype.getTxHistory = function (opts, cb) {
  ...
  opts.asset = (_.isUndefined(opts.asset) ? null : opts.asset);
  if (!Utils.checkAsset(opts.asset, cb)) {
    return;
  }

  opts.limit = (_.isUndefined(opts.limit) ? 5 : opts.limit);
  ...
  if (opts.last_rowid && !ValidationUtils.isPositiveInteger(opts.last_rowid))
