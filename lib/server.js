...
WalletService.prototype._sendMultiPayment = function (wallet, opts, cb) {
  ...
  self.storage.fetchAddresses(self.walletId, function (err, addresses) {
    ...
    var params = {
      ...
      spend_unconfirmed: opts.params.spend_unconfirmed || 'all',
 ...
 * @param {Boolean} opts.params.spend_unconfirmed [Optional] - UTXOs of unconfirmed transactions as inputs. (defaults to 'all', others include 'own', 'none')
 ...
 * @return {undefined}
 */
WalletService.prototype.removeTx = function (opts, cb) {
  var self = this;
  ...
  self._runLocked(cb, function (cb) {
    ...
    self.getTx({
      ...
      if (txp.isAccepted() || txp.isBroadcasted()) return cb(Errors.TX_CANNOT_REMOVE);
