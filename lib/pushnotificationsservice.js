...
PushNotificationsService.prototype.start = function(opts, cb) {
  ...
  self.defaultUnit = opts.pushNotificationsOpts.defaultUnit || 'one';
        ...
        try {
          ...
          if (data.asset) {
            self.storage.fetchAssetByUnit(opts.asset, function (err, result) {
              if (err) return next(err);
              if (_.isEmpty(result)) return next(new Error('No such asset'));
              data.amount = Utils.formatAmount(+data.amount, result, unit);
              next();
            });
          } else {
            data.amount = Utils.formatAmount(+data.amount, data.asset, unit);
            next();
          }
        } catch (ex) {
          return next(new Error('Could not format amount', ex));
        }
      } else {
        next();
      }
