...
BlockchainMonitor.prototype.start = function(opts, cb) {
  ...
  async.series([
    ...
    function(done) {
      if (conf.bLight) {
        done('blockchain monitor only support full node!');
      }
      done();
    },
    ...
    function(done) {
      myWitnesses.readMyWitnesses(function(arrWitnesses) {
        if (arrWitnesses.length > 0) {
          ...
          myWitnesses.insertWitnesses(conf.initial_witnesses, function() {
            self._startHub();
            done();
          });
        }
      ...
      function readMetadata(cb) {
        ...
        async.series([
          ...
          function (next) {
            ...
            self.storage.fetchAssets(opts, function (err, result) {
              if (err) return next(err);
...
BlockchainMonitor.prototype._handleNewJoint = function(objJoint) {
  ...
  async.waterfall([
    ...
    function (txp, next) {
      if (txp) {
        self.lock.runLocked(objUnit.unit, {}, 
          function(err) {
            if (err) {
          ...
          function() {
            ...
            self.storage.fetchNotifications(walletId, null, fromTs, function(err, notifications) {
              if (err) {
                log.error('Fetch notifications error:' + err);
                return next(err);
              }
              ...
              self.storage.storeTx(walletId, txp, function(err) {
                ...
                if (txp.app == 'payment') {
                  notification.data.asset = txp.params.asset;
                  notification.data.amount = txp.getTotalAmount();
                  
                  if (txp.params.asset == 'base' || txp.params.asset == null) {
                    notification.data.decimals = 0;
                    notification.data.ticker = 'BYTES';
                    ...
                    self.storage.fetchAssetByUnit(txp.params.asset, function (err, result) {
                      if (err || _.isEmpty(result)) {
                        notification.data.decimals = 0;
                        notification.data.ticker = 'UNKNOWN';
                        self._storeAndBroadcastNotification(notification, next);
                      } else {
                        notification.data.decimals = result[0].decimals;
                        notification.data.ticker = result[0].ticker;
                        self._storeAndBroadcastNotification(notification, next);
                      }
                    });
                  }
    ...
    function (next) {
      ...
      async.each(outputs, function(output, next2) {
        self.storage.fetchAddressByCoin('obyte', output.address, function(err, address) {
          ...
          if (!address || address.isChange) return next2();
          if (objUnit.authors.find(author => author.address==output.address)) return next2();
    
          var walletId = address.walletId;
          log.info('Incoming tx for wallet ' + walletId + ' [' + output.address + ']');
          ...
          self.storage.fetchNotifications(walletId, null, fromTs, function(err, notifications) {
            ...
            var alreadyNotified = _.some(notifications, function(n) {
              return n.type == 'NewIncomingTx' && n.data && n.data.txid == objUnit.unit && n.data.address == output.address && n.data.amount == output.amount;
            });
            if (alreadyNotified) {
              log.info('The incoming tx ' + objUnit.unit + ' was already notified');
              return next2();
            ...
            if (output.asset == 'base' || output.asset == null) {
              notification.data.decimals = 0;
              notification.data.ticker = 'BYTES';
              self._storeAndBroadcastNotification(notification, next2);
            } else {
              self.storage.fetchAssetByUnit(output.asset, function (err, result) {
                if (err || _.isEmpty(result)) {
                  notification.data.decimals = 0;
                  notification.data.ticker = 'UNKNOWN';
                  self._storeAndBroadcastNotification(notification, next2);
    ...
    function (next) {
      ...
      self._storeActiveAddresses(_.union(authorAddresses, outputAddresses), next);
    }
