...
Utils.formatAmount = function(amount, asset, unit, opts) {
  var UNITS = {
    giga: {
      toBytes: 1000000000,
      maxDecimals: 7,
  ...
  $.shouldBeNumber(amount);
  $.checkArgument(_.includes(_.keys(UNITS), unit));

  if (asset != null && !Utils.checkRequired(asset, ['decimals', 'ticker'])) {
  ...
  if (asset) {
    var n = (amount/Math.pow(10, asset.decimals)).toFixed(asset.decimals);
    return addSeparators(n, opts.thousandsSeparator || ',', opts.decimalSeparator || '.', asset.decimals) + ' ' + asset.ticker;
  } else {
    var u = _.assign(UNITS[unit], opts);
    var bytes = (amount / u.toBytes).toFixed(u.maxDecimals);
    return addSeparators(bytes, opts.thousandsSeparator || ',', opts.decimalSeparator || '.', u.minDecimals) + ' ' + UNIT_LABELS[unit];
  }
};

Utils.formatAmountInGiga = function(amount, asset) {
  return Utils.formatAmount(amount, asset, 'giga', {
    minDecimals: 9,
    maxDecimals: 9,
  });
};
