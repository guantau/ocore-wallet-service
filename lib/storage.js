...
Storage.prototype._createIndexes = function() {
  this.db.collection(collections.DEVICES).createIndex({
    id: 1
  });
...
Storage.prototype.fetchAllAddresses = function(cb) {
  var self = this;
  this.db.collection(collections.ADDRESSES).find().sort({
    createdOn: 1
  }).toArray(function(err, result) {
    if (err) return cb(err);
    if (!result) return cb();
...
Storage.prototype.storeAsset = function(asset, cb) {
  ...
  this.db.collection(collections.ASSETS).update({
    asset: asset.asset
  }, asset, {
    w: 1,
    upsert: true,
...
Storage.prototype.fetchAsset = function(asset, cb) {
  var self = this;
