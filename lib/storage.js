...
Storage.prototype._createIndexes = function() {
  ...
  this.db.collection(collections.ASSETS).createIndex({
    asset: 1
  });
  this.db.collection(collections.MESSAGES).createIndex({
    fromWalletId: 1,
    id: 1,
  });
  this.db.collection(collections.MESSAGES).createIndex({
...
Storage.prototype.storeWalletAndUpdateCopayersLookup = function(wallet, cb) {
  ...
  var copayerLookups = _.map(wallet.copayers, function(copayer) {
    ...
    return {
      deviceId: copayer.deviceId,
      account: copayer.account,
...
Storage.prototype.fetchCopayers = function(deviceId, cb) {
  this.db.collection(collections.COPAYERS_LOOKUP).find({
    deviceId: deviceId
...
Storage.prototype._completeTxData = function(walletId, txs, cb) {
  ...
  self.fetchWallet(walletId, function(err, wallet) {
    ...
    _.each([].concat(txs), function(tx) {
      tx.derivationStrategy = wallet.derivationStrategy || 'BIP44';
...
Storage.prototype.fetchTxs = function(walletId, opts, cb) {
  ...
  if (_.includes(['temporary', 'pending', 'accepted', 'broadcasted', 'rejected'], opts.status)) 
    filter.status = opts.status;
  if (opts.app) filter.app = opts.app;
  if (opts.isPending) filter.isPending = true;
...
Storage.prototype.fetchMessages = function(fromWalletId, toWalletId, opts, cb) {
  var self = this;

  var idFilter = {};
  if (opts.lastMessageId) {
    idFilter.$lt = opts.lastMessageId;
  }

  var filter = {};
  if (fromWalletId) filter.fromWalletId = fromWalletId;
  if (toWalletId) filter.toWalletId = toWalletId;
  if (!_.isEmpty(idFilter)) filter.id = idFilter;

  var mods = {};
  if (_.isNumber(opts.limit)) mods.limit = opts.limit;

  this.db.collection(collections.MESSAGES).find(filter, mods).sort({id: -1})
    .toArray(function(err, result) {
      if (err) return cb(err);
      if (!result) return cb(null, []);
      var messages = _.map(result, function(message) {
        return Model.Message.fromObj(message);
      });
      return cb(null, messages);
    });
};

Storage.prototype.storeMessage = function(message, cb) {
  this.db.collection(collections.MESSAGES).insert(message, {
    w: 1
  }, cb);
};
...
Storage.prototype.fetchAllAddresses = function(cb) {
  var self = this;
  self.db.collection(collections.ADDRESSES).find().sort({
    createdOn: 1
  }).toArray(function(err, result) {
    if (err) return cb(err);
    if (!result) return cb();
...
Storage.prototype.fetchNoActivityAddresses = function(walletId, isChange, cb) {
  var self = this;
  self.db.collection(collections.ADDRESSES).find({
    walletId: walletId,
    isChange: isChange,
...
Storage.prototype.markActiveAddresses = function(addresses, cb) {
  var self = this;
  self.db.collection(collections.ADDRESSES).update({
  ...
  self.db.collection(collections.ADDRESSES).find({
    ...
    if (result.length > 1) {
      result = _.find(result, function(address) {
        return coin == (address.coin || 'obyte');
      });
    } else {
...
Storage.prototype.storeAsset = function(asset, cb) {
  var self = this;
  self.db.collection(collections.ASSETS).update({
    asset: asset.asset
  }, asset, {
    w: 1,
    upsert: true,
...
Storage.prototype.fetchAssetByUnit = function(asset, cb) {
  var self = this;
  self.db.collection(collections.ASSETS).find({
    asset: asset
  }).sort({
    createdOn: 1
...
Storage.prototype.fetchAssets = function(opts, cb) {
  var self = this;
  self.db.collection(collections.ASSETS).find(opts).sort({
    createdOn: 1
  }).toArray(function(err, result) {
    if (err || _.isEmpty(result)) return cb(err);
    return cb(null, result);
...
Storage.prototype.removeSession = function(copayerId, cb) {
  this.db.collection(collections.SESSIONS).findAndRemove({
    copayerId: copayerId,
  }, {
    w: 1
  }, cb);
