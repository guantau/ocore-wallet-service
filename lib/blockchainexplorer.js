...
BlockchainExplorer.prototype.getUtxos = function(addresses, asset, handleUtxos) {
  ...
  db.query(
    "SELECT unit, message_index, output_index, address, amount, asset, denomination, is_stable AS stable, \n\
    "+ db.getUnixTimestamp("units.creation_date") + " AS time FROM outputs JOIN units USING(unit) \n\
    WHERE is_spent=0 AND address IN(?) AND sequence='good'" + getStrSqlFilterAssetForSingleTypeOfTransactions(asset) + " ORDER BY time DESC", 
