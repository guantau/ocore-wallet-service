...
ExpressApp.prototype.start = function(opts, cb) {
  ...
  router.get('/v1/device/', createWalletLimiter, function(req, res) {
    var opts = {};
  ...
  router.get('/v1/balance/', function(req, res) {
    getServerWithAuth(req, res, function(server) {
      ...
      if (req.query.asset) opts.asset = req.query.asset;
  ...
  router.post('/v1/broadcast_raw/', function(req, res) {
    getServerWithAuth(req, res, function(server) {
      server.broadcastJoint(req.body, function(err, txid) {
        if (err) return returnError(err, res, req);
        res.json(txid);
        res.end();
      });
