...
Wallet.prototype._updatePublicKeyRing = function() {
  this.publicKeyRing = _.map(this.copayers, function(copayer) {
    return _.pick(copayer, ['xPubKey', 'requestPubKey', 'deviceId']);
  });
};
...
Wallet.prototype.createAddress = function(isChange, step) {
  ...
  var addressType = this.n > 1 ? Constants.ADDRESS_TYPES.SHARED : Constants.ADDRESS_TYPES.NORMAL;
  var address = Address.derive(self.id, this.definitionTemplate, this.copayers, path, addressType, isChange);  
  return address;
};
...
Wallet.prototype.getSkippedAddress = function() {
  ...
  if (!next) return;
  var address = Address.derive(self.id, this.definitionTemplate, this.publicKeyRing, next.path, addressType, next.isChange);
  return address;
};
