 ...
 * @param {String} params.spend_unconfirmed [optional] - default 'own', or 'all', 'none'
...
function composeJoint(params) {
  ...
  async.series([
    ...
    function (cb) { // authors
      async.eachSeries(arrFromAddresses, function (from_address, cb2) {
        ...
        signer.readSigningPaths(from_address, function (err, assocLengthsBySigningPaths, addressInfo) {
          ...
          assocSigningInfo[from_address] = _.extend(addressInfo, {signingPaths : arrSigningPaths});
    ...
    function (cb) { // input coins
      console.log(objPaymentMessage.payload);
