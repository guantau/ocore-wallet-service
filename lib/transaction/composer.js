...
var parentComposer = require('ocore/parent_composer.js');
...
function createTextMessage(text) {
  return {
    ...
    payload_hash: objectHash.getBase64Hash(text, true),
    payload: text
  };
}
...
/** 
 * Compose a joint with bytes payment message
 * 
 * @param {Object} params
 * @param {Array} params.paying_addresses - pay for byte outputs and commissions
 * @param {Array} params.signing_addresses [optional] - must sign the message but they do not necessarily pay 
 * @param {Array} params.outputs - the output with amount=0 is used for change
 * @param {Array} params.witnesses [optional] - witness list
 * @param {Boolean} params.minimal [optional] - use as few paying_addresses as possible
 * @param {Boolean} params.send_all [optional] - send all bytes
 * @param {Array} params.inputs [optional] - inputs for the payment
 * @param {Number} params.input_amount [optional] - input amount
 ...
 * @param {Callback} params.retrieveMessages [optional] - add external messages, i.e. asset payment message
...
function composeJoint(params) {
  ...
  async.series([
    ...
    function (cb) { // authors
      async.eachSeries(arrFromAddresses, function (from_address, cb2) {
        ...
        function setDefinition() {
          signer.readDefinition(from_address, function (err, arrDefinition) {
        ...
        signer.readSigningPaths(from_address, function (err, assocLengthsBySigningPaths, addressInfo) {
          if (err) return cb2(err);
          var arrSigningPaths = Object.keys(assocLengthsBySigningPaths);
          assocSigningInfo[from_address] = addressInfo;
    ...
    function (cb) { // messages retrieved via callback
      if (!fnRetrieveMessages)
        return cb();
      console.log("will retrieve messages");
      fnRetrieveMessages(conn, last_ball_mci, bMultiAuthored, arrPayingAddresses, function (err, arrMoreMessages) {
    ...
    function (cb) { // input coins
      ...
      inputs.pickDivisibleCoinsForAmount(
        conn, null, arrPayingAddresses, last_ball_mci, target_amount, bMultiAuthored, params.spend_unconfirmed || 'all',
    ...
    conn.query(err ? "ROLLBACK" : "COMMIT", function () {
      ...
      objPaymentMessage.payload_hash = objectHash.getBase64Hash(objPaymentMessage.payload, true);

      objJoint.signingInfo = assocSigningInfo;

      _.each(objUnit.authors, function (author) {
...
function composeMinimalJoint(params) {
  ...
  readSortedFundedAddresses(null, params.available_paying_addresses, estimated_amount, params.spend_unconfirmed || 'all', function (arrFundedPayingAddresses) {
...
exports.composeMinimalJoint = composeMinimalJoint;
